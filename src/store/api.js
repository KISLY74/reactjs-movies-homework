...
export const getPopularMovies = createAsyncThunk(`moviesPopular`, () => {
  return axios.get(BASE_URL + `movie/popular`, config).then(res => res.data.results)
})

export const getTopRatedMovies = createAsyncThunk(`moviesTopRated`, () => {
  return axios.get(BASE_URL + 'movie/top_rated', config).then(res => res.data.results)
})

export const getUpcomingMovies = createAsyncThunk(`moviesUpcoming`, () => {
